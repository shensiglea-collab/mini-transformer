...
class Trainer:
    ...
    def __init__(self, model: nn.Module, model_name: str = "Model",
                 lr: float = 0.001, criterion: Optional[nn.Module] = None,
                 device: str = 'cpu'):
        """
        初始化训练器
    ...
    def evaluate(self, X_test: torch.Tensor, y_test: torch.Tensor,
        ...
        Args:
            X_test: 测试特征
            y_test: 测试目标（张量）
            y_test_numpy: 测试目标（numpy数组，用于计算指标）
...
def train_model(model: nn.Module, X_train: torch.Tensor, y_train: torch.Tensor,
    ...
    Returns:
        训练损失历史
    """
    trainer = Trainer(model, lr=lr, criterion=criterion, verbose=verbose)
    return trainer.train(X_train, y_train, num_epochs, batch_size, verbose)
